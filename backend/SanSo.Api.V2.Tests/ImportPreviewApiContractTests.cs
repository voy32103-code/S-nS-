...
public sealed class ImportPreviewApiContractTests
{
    [Fact]
    ...
    public async Task SameCsvIsReportedAsDuplicateForTheSameTenant()
    {
        ...
        Assert.Equal(firstBody.GetProperty("checksum").GetString(), secondBody.GetProperty("checksum").GetString());
    }
    ...
    private static async Task<HttpResponseMessage> Upload(HttpClient client, string fileName, string contentType, string contents)
    {
