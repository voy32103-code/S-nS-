...
public sealed class PostgresImportStore(NpgsqlDataSource dataSource)
{
    public async Task<PersistentImportPreview> Stage(string tenant, ImportPreview preview, CancellationToken ct)
    {
        ...
        batch.CommandText = """
            INSERT INTO import_confirmation_batches(organization_id,checksum,token_hash,format,template_version,status,expires_at)
        ...
        foreach (var row in preview.Rows)
        {
            ...
            insert.CommandText = """
                INSERT INTO import_confirmation_rows(organization_id,batch_id,row_number,event_id,normalized_payload,errors)
    ...
    public async Task<PersistentImportConfirmation> Confirm(string tenant, string previewToken, string checksum, string actorId, CancellationToken ct)
    {
        ...
        select.CommandText = """
            SELECT id::text,checksum,status,expires_at FROM import_confirmation_batches
        ...
        duplicate.CommandText = "SELECT id::text FROM import_confirmation_batches WHERE organization_id=$1::uuid AND checksum=$2 AND status='CONFIRMED' AND id<>$3::uuid";
        ...
        post.CommandText = """
            ...
            FROM import_confirmation_rows WHERE organization_id=$1::uuid AND batch_id=$2::uuid AND errors='[]'::jsonb
            ON CONFLICT(organization_id,source,source_event_id) DO NOTHING;
            UPDATE import_confirmation_batches SET status='CONFIRMED',confirmed_at=now(),confirmed_by=$3::uuid
        ...
        count.CommandText = "SELECT count(*) FILTER (WHERE errors='[]'::jsonb),count(*) FILTER (WHERE errors<>'[]'::jsonb) FROM import_confirmation_rows WHERE organization_id=$1::uuid AND batch_id=$2::uuid";
