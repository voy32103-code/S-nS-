...
app.UseMiddleware<SafeProblemMiddleware>();
app.UseCors();
...
{
    if (!Db()) return Results.Ok(new { status = "degraded", database = "not-configured", api = "v10" });
    try
    {
        ...
        return Results.Ok(new { status = "ok", database = "postgresql", api = "v10" });
    }
