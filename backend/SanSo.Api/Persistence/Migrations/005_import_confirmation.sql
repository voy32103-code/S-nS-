...
CREATE UNIQUE INDEX ux_import_batches_confirmed_checksum ON import_batches(organization_id,checksum) WHERE status='CONFIRMED';
CREATE TABLE import_rows (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES organizations(id),
  batch_id uuid NOT NULL REFERENCES import_batches(id) ON DELETE RESTRICT, row_number integer NOT NULL CHECK(row_number>0),
...
CREATE POLICY import_batches_tenant ON import_batches USING(organization_id=current_setting('app.current_organization_id',true)::uuid) WITH CHECK(organization_id=current_setting('app.current_organization_id',true)::uuid);
CREATE POLICY import_rows_tenant ON import_rows USING(organization_id=current_setting('app.current_organization_id',true)::uuid) WITH CHECK(organization_id=current_setting('app.current_organization_id',true)::uuid);
CREATE INDEX ix_import_batches_tenant_status_expiry ON import_batches(organization_id,status,expires_at);
