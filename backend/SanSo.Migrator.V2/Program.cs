...
{
    command.CommandText = "SELECT pg_advisory_lock(7482152027); CREATE TABLE IF NOT EXISTS schema_migrations_v2(version text PRIMARY KEY,checksum text NOT NULL,applied_at timestamptz NOT NULL DEFAULT now());";
...
try
{
    foreach (var file in Directory.GetFiles(migrationPath, "*.sql").OrderBy(Path.GetFileName, StringComparer.Ordinal))
    {
        ...
        check.CommandText = "SELECT checksum FROM schema_migrations_v2 WHERE version=$1";
        check.Parameters.AddWithValue(version);
        var existing = await check.ExecuteScalarAsync() as string;
        if (existing is not null)
        {
            if (existing != checksum) throw new InvalidOperationException($"Migration checksum changed after apply: {version}");
        ...
        record.CommandText = "INSERT INTO schema_migrations_v2(version,checksum) VALUES($1,$2)";
        record.Parameters.AddWithValue(version);
        record.Parameters.AddWithValue(checksum);
        await record.ExecuteNonQueryAsync();
        await transaction.CommitAsync();
        Console.WriteLine($"APPLIED {version} {checksum}");
    }
}
finally
{
