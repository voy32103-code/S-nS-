...
test('login has no WCAG A/AA violations',async({page})=>{await page.goto('/index-v8.html');const result=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa']).analyze();expect(result.violations).toEqual([])});

test('authenticated overview has no WCAG A/AA violations',async({page})=>{await page.goto('/index-v8.html');const inputs=page.locator('form input');await inputs.nth(0).fill(email);await inputs.nth(1).fill(password);await inputs.nth(2).fill(tenant);await inputs.nth(3).fill(totp());await page.locator('form button').click();await expect(page.locator('aside')).toBeVisible();await expect(page.getByText(/SET-/)).toBeVisible();const result=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa']).analyze();expect(result.violations).toEqual([])});
