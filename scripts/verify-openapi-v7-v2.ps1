param(
    [string]$Path = "docs/openapi-v7-canonical.json",
    [string]$EntryPoint = "backend/SanSo.Api.V6/ProgramCanonicalV7.cs"
)
...
foreach ($route in $required) {
    ...
    if ($route -ne "/api/auth/login" -and -not $source.Contains($route)) { throw "ROUTE_NOT_IN_CANONICAL_SOURCE:$route" }
}
...
Write-Output "OPENAPI_V7_VERIFIED paths=$pathCount refs=$($refs.Count) max_upload=$($file.maxLength) source_routes=$($required.Count)"
